...
from utils.DockerClient import DockerClient
...
class TestFailoverFunc:
    ...
    def setup_class(self):
        ...
        self.docker_c = DockerClient(conf_obj)
        self.retry = Retry(conf_obj)
        ...
        self.sql_c = SQLClient(conf_obj['mysql_host'], conf_obj["mysql_port"], conf_obj["mysql_user"],
                               ...
                               conf_obj["mysql_db"])
        self.ca = CreateArgs(data_obj['capacity'], data_obj['zoneId'], data_obj['remarks'], data_obj['spaceName'],
                             data_obj['spaceType'], data_obj['quantity'])
        self.space_id, self.space_info = CreateCluster(self.wc, self.ca, self.sql_c)
    ...
    def test_failover_recreate_slave(self):
        ...
        print "[STEP] Search slave info of instance (space_id={0}). Slave Info : IP:Port={1}:{2}".format(self.space_id,
                                                                                                         slave_ip,
                                                                                                         slave_port)

        # 链接docker服务器的守护进程，根据IP_PORT停止slave
        print "[STEP] Stop slave of cache instance （space_id={0})".format(self.space_id)
        self.docker_c.stop_container(slave_ip, slave_port)
        ...
        assert slave_ip_new != slave_ip or slave_port_new != slave_port
    ...
    def test_failover_recreate_master(self):
        ...
        master_info_new = self.retry.retry_get_new_container(self.space_id,"master")
        master_ip_new = master_info_new[0]
        master_port_new = master_info_new[1]

        # 验证点:faiover创建的新的master的Port与最初的master的Port不同
        assert master_ip_new != master_ip or master_port_new != master_port
        ...
        master_ip = master_info[0]
        master_port = master_info[1]
        print "[STEP] 向master({0}:{1})写入key-value=（{2}-{3}）".format(master_ip, master_port, "cache_test_key_1",
                                                                    "cache_test_value_1")
