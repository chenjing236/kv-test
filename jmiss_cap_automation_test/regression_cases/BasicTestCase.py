...
def create_mongo_instance_three(request, config, instance_data, mongo_http_client, cap_http_client):
    ...
    # 创建mongo实例
    request_id_for_mongo = create_mongo_instance_step(config, instance_data, mongo_http_client)
    request_id_for_mongo2 = create_mongo_instance_step(config, instance_data, mongo_http_client)
    request_id_for_mongo3 = create_mongo_instance_step(config, instance_data, mongo_http_client)
    info_logger.info("[INFO] The mongo instance is created, and the request id is %s", request_id_for_mongo)
    # 支付
    info_logger.info("[STEP] Pay for the mongo instance")
    request_id_for_paying_mongo = pay_for_mongo_instance_step(config, instance_data, cap_http_client, request_id_for_mongo)
    request_id_for_paying_mongo2 = pay_for_mongo_instance_step(config, instance_data, cap_http_client, request_id_for_mongo2)
    request_id_for_paying_mongo3 = pay_for_mongo_instance_step(config, instance_data, cap_http_client, request_id_for_mongo3)
    info_logger.info("[INFO] The request id is %s for paying mongo", request_id_for_paying_mongo)
    # 查询订单状态
    info_logger.info("[STEP] Get the status of the order for the mongo instance")
    success, resource_id = query_order_status_for_mongo_step(config, instance_data, cap_http_client, request_id_for_mongo)
    success2, resource_id2 = query_order_status_for_mongo_step(config, instance_data, cap_http_client, request_id_for_mongo2)
    success3, resource_id3 = query_order_status_for_mongo_step(config, instance_data, cap_http_client, request_id_for_mongo3)
    info_logger.info("[INFO] The resource id is %s for the mongo", resource_id)
    info_logger.info("[INFO] The resource id is %s for the mongo", resource_id2)
    info_logger.info("[INFO] The resource id is %s for the mongo", resource_id3)
    # 查询详情接口
    info_logger.info("[STEP] Get the detail info of the mongo instance")
    request_id, mongo_info = query_mongo_db_detail_step(config, instance_data, mongo_http_client, resource_id)
    if mongo_info is None:
    ...
    request_id2, mongo_info2 = query_mongo_db_detail_step(config, instance_data, mongo_http_client, resource_id2)
    if mongo_info2 is None:
        info_logger.info("[ERROR] The mongo instance %s is not be created", resource_id2)
    assert False, "[ERROR] The mongo instance %s is not be created".format(resource_id2)
    assert mongo_info2["status"] == 100, "[ERROR] The mongo instance %s is not be created".format(resource_id2)

    request_id3, mongo_info3 = query_mongo_db_detail_step(config, instance_data, mongo_http_client, resource_id3)
    if mongo_info3 is None:
        info_logger.info("[ERROR] The mongo instance %s is not be created", resource_id3)
    assert False, "[ERROR] The mongo instance %s is not be created".format(resource_id3)
    assert mongo_info3["status"] == 100, "[ERROR] The mongo instance %s is not be created".format(resource_id3)
