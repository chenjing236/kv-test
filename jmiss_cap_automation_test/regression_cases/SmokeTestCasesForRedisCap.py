...
class TestSmokeCasesForRedisCap:
    ...
    def test_query_filter_cache_clusters(self, create_redis_instance, create_redis_month_instance):
        ...
        for cluster in clusters:
            ...
            assert "cap_auto_test" in cluster[
                "name"], "[ERROR] QueryFilterCacheClusters use filter category=1 is incorrect!"
            assert cluster["billingOrder"][
                       "category"] == 1, "[ERROR] QueryFilterCacheCluster use filter category=6 is incorrect"
        ...
        for cluster in clusters:
            assert ("cap_auto_test") in cluster[
                "name"], "[ERROR] QueryFilterCacheClusters use filter category=6 is correct!"
            assert cluster["billingOrder"][
                       "category"] == 6, "[ERROR] QueryFilterCacheCluster use filter category=6 is incorrect"
    ...
    def test_renew_month_redis(self, create_redis_month_instance):
        ...
        info_logger.info(
            "[INFO] Check the feeType of the redis instance successfully, the feeType = {0}".format(fee_type))
        ...
        assert len(
            statusByResourceIdResponseList) == 0, "[ERROR] The status of the redis is wrong for the list of resource status is not null"
