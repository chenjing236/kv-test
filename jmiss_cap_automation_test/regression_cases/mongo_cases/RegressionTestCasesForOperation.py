...
class TestSmokeCasesForOperation:
	...
	# 验证包年包月资源在列表中
	# 运营系统删除包年包月资源
	request_id = delete_resource_step(config, instance_data, http_client, resource_id, instance_data["create_mongo_db_with_yearly_fee"]["feeType"])
	# 查看资源列表
	mongo_info_list = query_mongo_dbs_step(config, instance_data, mongo_http_client)
	# 验证被删除的资源不在资源列表信息中
	# 创建mongo实例，类型为按计费类型
    ...
    # 运营系统，删除未过期资源
    @pytest.mark.smoke
    def test_deleteNoOverdueResource(self, config, instance_data, cap_http_client, create_mongo_instance, create_mongo_instance_with_yealy_fee):
        # 创建mongo实例,类型为包年包月的资源
