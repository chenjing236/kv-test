...
class TestOperationQuota:
    ...
    def test_operation_quota(self, config, instance_data, cap_http_client):
        ...
        resource = "redis"
        request_id, res_data_total, res_data_use = query_user_quota_step(cap, resource)
        if res_data_total > 0:
        ...
        time.sleep(1)
        request_id, quota, use = modify_used_quota_step(cap, resource, res_data_total)
        if quota > 0:
        ...
        time.sleep(1)
        request_id, quota, use = modify_used_quota_step(cap, resource, res_data_use)
        if quota > 0:
            assert quota >= use, "[INFO] Query user quota is incorrect,used quota !> total quota"
        info_logger.info("[INFO] Modify user quota successfully, the used quota is {0} now".format(use))
