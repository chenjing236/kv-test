...
pytest ./test_cases/openapi_cases -s -v -m openapi --config ./config/conf_sh.json --data ./data/instance_data_sh.json > /export/Logs/mc_stability_test/mc_stability_sh_$date.log 2>&1
...
re=`cat /export/Logs/mc_stability_test/mc_stability_sh_${date}.log|grep -E 'PASSED|FAILED'`
i=0
for ca in ${re[@]}; do
        if [[ $ca =~ 'PASSED' ]]; then
