...
class TestResizeCluster:
    ...
    def test_resize_cluster_to_larger_cluster(self, config, instance_data, http_client, created_cluster):
        # 创建缓存云实例
        ...
        assert cluster_detail["cacheInstanceClass"] != instance_data["cache_cluster_class_resize"]
        # domain = cluster_detail["connectionDomain"]
        # 验证通过nlb访问实例
        # check_access_domain_step(accesser, space_id, password)
        # 执行扩容操作
        cache_cluster_class_resize = instance_data["cache_cluster_class_resize"]
        error = resize_step(redis_cap, space_id, cache_cluster_class_resize)
        assert error is None
        # 查看redis详情，验证缓存云实例状态，status=running, 验证基本信息正确
        cluster_detail_new, error = query_detail_step(redis_cap, space_id)
        assert cluster_detail_new["cacheInstanceStatus"] == "running"
        assert cluster_detail_new["cacheInstanceClass"] == instance_data["cache_cluster_class_resize"]
        # 验证通过nlb访问实例
        # check_access_domain_step(accesser, space_id, password)
    ...
    @pytest.mark.regression
    def test_resize_cluster_to_smaller_cluster(self, config, instance_data, http_client, created_cluster):
        # 创建缓存云实例
        space_id, redis_cap, password, accesser = created_cluster
        # 查看redis详情，验证缓存云实例状态，status=running, 验证基本信息正确
        cluster_detail, error = query_detail_step(redis_cap, space_id)
        assert cluster_detail["cacheInstanceStatus"] == "running"
        assert cluster_detail["cacheInstanceClass"] != instance_data["cache_cluster_class_reduce"]
        # domain = cluster_detail["connectionDomain"]
        # 验证通过nlb访问实例
        # check_access_domain_step(accesser, space_id, password)
        # 执行扩容操作
        cache_cluster_class_reduce = instance_data["cache_cluster_class_reduce"]
        error = resize_step(redis_cap, space_id, cache_cluster_class_reduce)
        assert error is None
        # 查看redis详情，验证缓存云实例状态，status=running, 验证基本信息正确
        cluster_detail_new, error = query_detail_step(redis_cap, space_id)
        assert cluster_detail_new["cacheInstanceStatus"] == "running"
        assert cluster_detail_new["cacheInstanceClass"] == instance_data["cache_cluster_class_reduce"]
        # 验证通过nlb访问实例
        # check_access_domain_step(accesser, space_id, password)
        capacity = cluster_detail_new["cacheInstanceMemoryMB"]
        # 查看缓存云实例详情，获取拓扑结构
    ...
    @pytest.mark.regression
    def test_resize_cluster_to_ms(self, config, instance_data, http_client, created_cluster):
        # 创建缓存云实例
        space_id, redis_cap, password, accesser = created_cluster
        # 查看redis详情，验证缓存云实例状态，status=running, 验证基本信息正确
        cluster_detail, error = query_detail_step(redis_cap, space_id)
        assert cluster_detail["cacheInstanceStatus"] == "running"
        assert cluster_detail["cacheInstanceClass"] != instance_data["cache_instance_class"]
        # domain = cluster_detail["connectionDomain"]
        # 验证通过nlb访问实例
        # check_access_domain_step(accesser, space_id, password)
        # 执行扩容操作
        cache_instance_class = instance_data["cache_instance_class"]
        error = resize_step(redis_cap, space_id, cache_instance_class)
        assert error is None
        # 查看redis详情，验证缓存云实例状态，status=running, 验证基本信息正确
        cluster_detail_new, error = query_detail_step(redis_cap, space_id)
        assert cluster_detail_new["cacheInstanceStatus"] == "running"
