...
pytest -x --config ./config/conf_pro_gz.json -m smoke > /export/Logs/redis40_stability_test/redis40_stability_gz_$date.log 2>&1
...
re=`cat /export/Logs/redis40_stability_test/redis40_stability_gz_${date}.log|grep -E 'PASSED|FAILED$'`
i=0
for ca in ${re[@]}; do
        if [[ $ca =~ 'PASSED' ]]; then
