...
class admimParam(object):
    def __init__(self, side, current_rs_type, next_rs_type, is_first_start, topo=None, password, max_memory, space_status,
        ...
        self.failovering_num = failovering_num
        self.shard_status = shard_status
        self.config_param = config_param
        self.admin_flavor = admin_flavor
        self.auto_backup_timer = auto_backup_timer
...
def get_shard_status(instanceId, config):
    _, _, resp = HttpClient.underlayEntry(config, instanceId, "GET", "/getSpace")
    return re.findall("status:\"(.*?)\"", resp["data"]["topo"]["shards"])
...
def check_all_admin(instanceId, config, excepted):
    ...
    if isCurrect:
        raise ValueError(
            "check is_first_start error,excepted.is_first_start=%s,actual is_first_start=%s" % excepted.is_first_start,
            actual)
    ...
    if isCurrect:
        raise ValueError("check max_memory error,excepted.max_memory=%s,actual max_memory=%s" % excepted.max_memory,
                         actual)

    isCurrect, actual = check_admin_param(instanceId, config, excepted.space_status, get_space_status)
    if isCurrect:
        raise ValueError(
            "check space_status error,excepted.space_status=%s,actual space_status=%s" % excepted.space_status, actual)

    isCurrect, actual = check_admin_param(instanceId, config, excepted.failovering_num, get_failovering_num)
    if isCurrect:
        raise ValueError(
            "check failovering_num error,excepted.failovering_num=%s,actual failovering_num=%s" % excepted.failovering_num,
            actual)

    isCurrect, actual = check_admin_param(instanceId, config, excepted.shard_status, get_shard_status)
    if isCurrect:
        raise ValueError(
            "check shard_status error,excepted.shard_status=%s,actual shard_status=%s" % excepted.shard_status, actual)
    ...
    if isCurrect:
        raise ValueError(
            "check admin_flavor error,excepted.admin_flavor=%s,actual admin_flavor=%s" % excepted.admin_flavor, actual)

    isCurrect, actual = check_admin_param(instanceId, config, excepted.auto_backup_timer, get_auto_backup_timer)
    if isCurrect:
        raise ValueError(
            "check auto_backup_timer error,excepted.auto_backup_timer=%s,actual auto_backup_timer=%s" % excepted.auto_backup_timer,
            actual)

    isCurrect, actual = check_admin_param(instanceId, config, excepted.backup_list, get_backup_list)
    if isCurrect:
        raise ValueError("check backup_list error,excepted.backup_list=%s,actual backup_list=%s" % excepted.backup_list,
                         actual)
