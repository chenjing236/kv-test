...
def check_all_admin(instanceId, config, excepted):
    ...
    if isCurrect:
        raise ValueError(
            "check is_first_start error,exceptedis_first_start=%s,actual is_first_start=%s" % (excepted["is_first_start"],actual))
    ...
    if isCurrect:
        raise ValueError(
            "check failovering_num error,exceptedfailovering_num=%s,actual failovering_num=%s" % (excepted["failovering_num"],actual))
    ...
    if isCurrect:
        raise ValueError(
            "check auto_backup_timer error,excepted.auto_backup_timer=%s,actual auto_backup_timer=%s" % (excepted["auto_backup_timer"],actual))

    isCurrect, actual = check_admin_param(instanceId, config, excepted["backup_list"], get_backup_list)
    if isCurrect:
        raise ValueError("check backup_list error,excepted.backup_list=%s,actual backup_list=%s" % (excepted["backup_list"],actual))
