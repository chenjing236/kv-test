...
def get_redis_maxmemory(instanceId, config):
    ...
    findResult=re.findall("maxmemory:(.*?)\r\n", resp)
    if len(set(findResult))!=1:
        raise ValueError("maxmemory is not same or find failed")
    return findResult[0]
...
def get_repl_backlog_size(instanceId, config):
    ...
    _, _, resp = HttpClient.underlayEntry(config, instanceId, "POST", "/config/redis", data)
    findResult =re.findall("repl_backlog_size:(.*?)\r\n", resp)
    if len(set(findResult)) != 1:
        raise ValueError("repl_backlog_size is not same or find failed")
    return findResult[0]
...
def get_maxmemory_policy(instanceId, config):
    data = {"type": "config", "commands": "info memory"}
    _, _, resp = HttpClient.underlayEntry(config, instanceId, "POST", "/config/redis", data)
    findResult=re.findall("maxmemory_policy:(.*?)\r\n", resp)
    if len(set(findResult))!=1:
        raise ValueError("maxmemory_policy is not same or find failed")
    return findResult[0]
...
def get_master_aof_enabled(instanceId, config):
    ...
    _, _, resp = HttpClient.underlayEntry(config, instanceId, "POST", "/config/redis", data)
    findResult =re.findall("aof_enabled:(.*?)\r\n", resp["result"]["masters"][0])
    if len(set(findResult)) != 1:
...
def get_slave_aof_enabled(instanceId, config):
    ...
    findResult = re.findall("aof_enabled:(.*?)\r\n", resp["result"]["slaves"][0])
    if len(set(findResult)) != 1:
        raise ValueError("slave aof_enabled is not same or find failed")
    return findResult[0]
...
def check_redis_list(instanceId, config, excepted, getConfigFunc):
    results = getConfigFunc(instanceId, config)
    return sorted(results) == sorted(excepted)


def check_all_redis(instanceId, config, excepted):
