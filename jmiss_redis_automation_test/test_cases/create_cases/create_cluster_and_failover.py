...
class TestCreateClusterFailover:
    ...
    # 1个redis-master、redis-slave 同时发生failover
    @staticmethod
    def _slave_master_failover_validation(config, instance, expected_object, instance_id):
        current_rs_type = get_current_rs_type(instance_id, config)
        redisNum = get_redis_num(instance_id, config, current_rs_type)
        redisId = get_shard_id(redisNum, 1)[0]
    ...
    # 1个proxy、redis-master 同时发生failover
    @staticmethod
    def __master_proxy_failover_validation(config, instance, expected_object, instance_id):
        current_rs_type = get_current_rs_type(instance_id, config)
        redisNum = get_redis_num(instance_id, config, current_rs_type)
        redisId = get_shard_id(redisNum, 1)[0]
    ...
    # 1个proxy、redis-slave 同时发生failover
    @staticmethod
    def __slave_proxy_failover_validation(config, instance, expected_object, instance_id):
        current_rs_type = get_current_rs_type(instance_id, config)
        redisNum = get_redis_num(instance_id, config, current_rs_type)
        redisId = get_shard_id(redisNum, 1)[0]
    ...
    # 1个proxy、redis-master、redis-slave同时发生failover
    @staticmethod
    def __slave_master_proxy_failover_validation(config, instance, expected_object, instance_id):
        current_rs_type = get_current_rs_type(instance_id, config)
        redisNum = get_redis_num(instance_id, config, current_rs_type)
        redisId = get_shard_id(redisNum, 1)[0]
    ...
    def __all_nodes_failover_validation(config, instance, expected_object, instance_id):
        ...
        masterName = replicasetMaster + "-" + str(redisId)
        slaveName = replicasetSlave + "-" + str(redisId)
        replicasetadmin = instance_id + "-admin"
        dockerAdmin = replicasetadmin + "-0"
        oldAdminRunTime = get_docker_running_time(config, instance_id, replicasetadmin, dockerAdmin)
        proxyId = get_shard_id(get_proxy_num(instance_id, config), 1)[0]
        replicasetName = instance_id + "-proxy"
        dockerName = replicasetName + "-" + str(proxyId)
        oldMasterRunTime = get_docker_running_time(config, instance_id, replicasetMaster, masterName)
        ...
        assert wait_docker_run_time_change(config, instance_id, oldAdminRunTime, replicasetadmin, dockerAdmin)
        assert wait_docker_run_time_change(config, instance_id, oldMasterRunTime, replicasetMaster, masterName)
        assert wait_docker_run_time_change(config, instance_id, oldSlaveRunTime, replicasetSlave, slaveName)
        assert wait_docker_run_time_change(config, instance_id, oldRunTime, replicasetName, dockerName)
    ...
    def test_cluster_failover_with_psw(self, config, instance_data, expected_data):
        instance = instance_data["create_cluster_specified"][0]
        ...
        self.__master_proxy_failover_validation(config, instance, expected_object, instance_id)
        self.__slave_proxy_failover_validation(config, instance, expected_object, instance_id)
        self.__slave_master_proxy_failover_validation(config, instance, expected_object, instance_id)
    ...
    # 1个proxy、redis-master、redis-slave同时发生failover
    # 所有节点同时发生failover
    def test_cluster_failover_without_psw(self, config, instance_data, expected_data):
        instance = instance_data["create_cluster_specified"][1]
        expected_object = baseCheckPoint(expected_data[instance["cacheInstanceClass"]], instance["instance_password"])
