...
from jmiss_redis_automation_test.steps.FailoverOperation import trigger_redis_failover, trigger_proxy_failover, \
    trigger_docker_failover
...
class TestStandardToStandardFailover:
    ...
    def test_source_master_failover(self, config, instance_data, expected_data):
        ...
        resp = reset_class(config, instanceId, "redis.s.small.basic", client,1)
        assertRespNotNone(resp)
        expected_object = baseCheckPoint(expected_data["redis.s.small.basic"],instances[0]["instance_password"])
        ...
        # 手动调用任务恢复接口
        # curl http://127.0.0.1:1818/reloadTask -d '{"taskId":"","isRollback":false}'
    ...
    def test_source_slave_failover(self, config, instance_data, expected_data):
        ...
        assert check_admin_proxy_redis_configmap(instanceId,config,expected_object,1)
    # 1个proxy failover
    @pytest.mark.resizeMasterFailover
    ...
    def test_source_master_and_proxy_failover(self, config, instance_data, expected_data):
        ...
        proxyDockerName = replicasetName + "-" + str(proxyId)
