# coding:utf-8
from BasicTestCase import *


class TestResetPassword:
    @pytest.mark.smoke
    @pytest.mark.regression
    def test_reset_password(self, created_instance):
        # 创建缓存云实例，创建成功
        space_id, instance, password_default, accesser = created_instance
        # 通过domain访问缓存云实例，输入auth默认token,可以正常访问
        check_access_domain_step(accesser, space_id, password_default)

        # run reset password
        password_new = "1qaz2WSX"
        time.sleep(3)
        reset_password_step(instance, space_id, password_new)
        # 使用长度为8的新密码，通过domain访问缓存云实例
        check_access_domain_step(accesser, space_id, password_new)

        # run reset password
        time.sleep(3)
        reset_password_step(instance, space_id, password_new + password_new)
        # 使用长度为16的新密码，通过domain访问缓存云实例
        check_access_domain_step(accesser, space_id, password_new + password_new)

        # run reset password
        time.sleep(3)
        # 待免密jmiss-web上线后，将下面参数password_default改为空字符串即可，此case为验证免密访问
        reset_password_step(instance, space_id, "")
        # 设置redis免密，通过domain访问缓存云实例
        check_access_domain_step(accesser, space_id)
